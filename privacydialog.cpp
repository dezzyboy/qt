                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zGST ought to be enough for anybody." - Bill Gates, 2017
    ui->zGSTpayAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzGSTSyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzGST->setEnabled(false);
        ui->pushButtonMintzGST->setToolTip(tr("zGST is currently disabled due to maintenance."));

        ui->pushButtonSpendzGST->setEnabled(false);
        ui->pushButtonSpendzGST->setToolTip(tr("zGST is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zGSTpayAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzGST_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGST is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zGST...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zGST in ") +
...
void PrivacyDialog::on_pushButtonSpendzGST_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zGST is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zGST
        sendzGST();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zGST
    sendzGST();
}

void PrivacyDialog::on_pushButtonZGHOSTControl_clicked()
{
    ZGHOSTControlDialog* zGHOSTControl = new ZGHOSTControlDialog(this);
    zGHOSTControl->setModel(walletModel);
    zGHOSTControl->exec();
}

void PrivacyDialog::setZGHOSTControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzGHOSTSelected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzGST()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid GHOST Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zGSTpayAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zGST is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " GST </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zGST</b>";
    ...
    // use mints from zGHOST selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZGHOSTControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZGHOSTControlDialog::GetSelectedMints();
    }
    ...
    if (!fSuccess) {
        ...
        const int nMaxSpends = Params().Zerocoin_MaxSpendsPerTransaction(); // Maximum possible spends for one zGST transaction
    ...
    // Clear zGHOST selector in case it was used
    ZGHOSTControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zGHOST Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " GHOST, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zGHOST Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zGST </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zGST "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zGST "));
    ui->labelzGSTAmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzGSTSyncStatus->setVisible(fShow);
}
