...
// Copyright (c) 2017-2018 The GHOST developers
// Distributed under the MIT/X11 software license, see the accompanying
// file COPYING or http://www.opensource.org/licenses/mit-license.php.

#include "zGHOSTcontroldialog.h"
#include "ui_zGHOSTcontroldialog.h"
...
std::list<std::string> ZGHOSTControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZGHOSTControlDialog::listMints;

ZGHOSTControlDialog::ZGHOSTControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZGHOSTControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZGHOSTControlDialog::~ZGHOSTControlDialog()
{
    delete ui;
}

void ZGHOSTControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZGHOSTControlDialog::updateList()
{
...
void ZGHOSTControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZGHOSTControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZGHOST_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZGHOSTControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZGHOSTControlDialog::GetSelectedMints()
{
...
void ZGHOSTControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
